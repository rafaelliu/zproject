...
public class Weld {
     ...
     */
    public WeldContainer initialize(ClassLoader classloader) {
        ResourceLoader resourceLoader = new ClassLoaderResourceLoader(classloader);
        // we won't check for beans.xml, every module is CDI
        ...
        Bootstrap bootstrap = new ForwardingBootstrap() {
            ...
            public Iterable<Metadata<Extension>> loadExtensions(ClassLoader classLoader) {
            	// TODO: currently we don't allow Extensions on modules. That is because
            	// loading ZContainerExtension (present in the service file) would trigger
            	// a new Container being created at @Observes AfterBeanDiscovery.
